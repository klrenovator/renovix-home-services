...
| `npm run audit:subservices` | Sub-service audit — lists the 51 priced sub-services, verifies authored pages are unique, belong to a real service, reference a real `pricingId` and carry all three language blocks, and (Phase 28 + Phase 29) that the service pillar → sub-service page, problem → sub-service and area guide → sub-service link wiring stays registry-derived and language-filtered |
...
| `npm run audit:multilingual` | EN/MS/ZH coverage complete; stale 18/28 location counts cannot return; no component may build display text by humanizing a slug (that is how English labels reached `/ms/` and `/zh/` pages) |
| `npm run audit:routes` | App Router tree matches the intended public architecture; no Carpentry category |
| `npm run audit:live` | Live QA against a running `next start` (all 678 sitemap URLs fetched, SEO/schema spot checks, quote API, security headers, and — Phase 28/29 — the rendered internal link graph: no orphan pages, every sub-service page linked from its own service pillar and back, every area guide linked to the sub-service scopes carried out there, no English slug label anywhere on `/ms/` or `/zh/`, no internal link to an unserved URL) |
