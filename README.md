...
| `npm run audit:blog` | Blog audit — every related slug and pricing id resolves, no hard-coded prices, EN/MS/ZH complete, unique metadata per language, no orphan articles; both region hubs derive a non-empty guide list from their own child areas and render it through the shared `GuideLinksSection` |
...
| `npm run audit:schema` | Structured-data honesty — required entity types present, no reviews/ratings/awards/opening days; pillar OfferCatalogs must map the full visible scope list without a sample cap, and sub-service Service nodes must reference the shared business provider and their own localized parent Service (Phase 43; rendered parity enforced by `audit:live`) |
