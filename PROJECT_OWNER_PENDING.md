...
| 5 | ◐ PART DONE 2026-09-06 — 7 owner photos wired as published projects (en/ms/zh + OG); remaining: owner to confirm provenance of the 2 painting shots, and supply real Waterproofing photos | Owner → `public/images/projects/` + `data/project-content/` | Raw jpgs on `main` were replaced by audited webp (originals recoverable from git history). Painting pair shows US-style fittings — not published until owner confirms a genuine local job. Never stock/AI images |
