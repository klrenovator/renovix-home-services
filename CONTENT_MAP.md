...
on a `/ms/` or `/zh/` page, no internal link to an unserved URL, and — since
Phase 39 — every pillar and location page rendering its in-copy links in all
three languages with no leaked `[label](/path)` markup anywhere).
